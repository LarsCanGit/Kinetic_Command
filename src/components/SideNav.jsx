...
const NAV_ITEMS = [
  { icon: 'grid_view', label: 'Board', page: 'board', fill: true },
  { icon: 'database', label: 'Database', page: 'database' },
]

export default function SideNav({ currentProject, onNewTask, open, onClose, onOpen, currentPage, onNavigate }) {
  if (!open) {
    return (
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 transition-all duration-100 ${
                  isActive
                    ? 'text-cyan-400 bg-[#002331] font-bold border-r-2 border-cyan-400'
                    : 'text-slate-400 hover:bg-[#002331]/50 hover:text-cyan-200'
                }`}
              >
                <span
